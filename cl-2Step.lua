...
-- ── Core effect function: flames + bang sound ─────────────────────────────────
local function fireEffect(veh)
    if not DoesEntityExist(veh) then return end

    local pos = GetEntityCoords(veh)

    -- Sound: use a proper audible explosion type with 0 damage/fire/physics
    -- Type 59 = grenade explosion sound only, no damage, widely confirmed in FiveM
    AddExplosion(pos.x, pos.y, pos.z, 59, 0.0, true, false, 0.0, false)

    -- Particles on exhaust bones
    if ptfxLoaded then
